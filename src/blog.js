...
async function fetchAllPosts() {
  ...
  for (const slug of BLOG_POSTS) {
    try {
      const markdown = await fetchMarkdownText(`/content/blog/${slug}.md`);
...
async function initBlogPost() {
  try {
    ...
    renderer.link = function(token) {
      ...
      const safeHref = sanitizeMarkdownLinkHref(token.href);
    ...
    renderer.image = function(token) {
      const safeSrc = sanitizeMarkdownImageSrc(token.href);
      if (!safeSrc) return '';
